...
dotnet run        # 编译并启动
```
或先 `dotnet build`，再运行 `bin\...\ApiTester.exe`。需要 .NET 10 桌面运行时（本机已装）。窗口打开时**自动屏幕居中**。
...
### 顶部
- **Protocol**：协议。切换会清空 Model 列表；选 Claude 时 `temp` 会被禁用（该模型不接受 temperature）。
...
- **temp**：temperature 采样温度（**留空则不发送**）。Claude 协议下禁用。
...
### 中部
- **Request (preview)**：发送前**实时预览**将要发出的完整请求（方法 + 完整 URL + 头 + body）。默认对 Key **脱敏**显示。
  - **Show key**：勾选后预览里显示**真实 Key**（便于整条复制排查）。
...
**保存到哪里**：
```
%APPDATA%\ApiTester\presets.json
即  C:\Users\<你的用户名>\AppData\Roaming\ApiTester\presets.json
```
文件是 JSON，形如：
```json
[
  { "Name": "my-openai", "Kind": "OpenAiChat", "BaseUrl": "https://api.openai.com", "ApiKey": "sk-xxxx" }
]
```
